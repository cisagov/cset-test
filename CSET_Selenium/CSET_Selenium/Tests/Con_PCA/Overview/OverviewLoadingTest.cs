...
namespace CSET_Selenium.Tests.Con_PCA.OverviewTest
{
    ...
    public class OverviewTest : BaseTest
    {
        ...
        public void OverviewAllTabsTest()
        {
            ...
            Assert.That(overview.IsPageLoaded(waitForSeconds), "Aggregate Statistics Page is not loaded in limited time.");
            ...
            Assert.That(overview.IsPageLoaded(waitForSeconds), "Subscription Status Page is not loaded in limited time.");

            Assert.That(overview.CheckIfElementExists(overview.GetSubscriptionStatusEndingSoonTable(), 3), "Subscription Ending Soon table is not loaded.");
            Assert.That(overview.CheckIfElementExists(overview.GetSubscriptionStatusInProgressTable(), 3), "Subscription In Progress table is not loaded.");
            Assert.That(overview.CheckIfElementExists(overview.GetSubscriptionStatusStoppedTable(), 3), "Subscription Stopped table is not loaded.");

            /* Logging Errors Tab*/
            overview.ClickLoggingErrorsTab();
            Assert.That(overview.IsPageLoaded(waitForSeconds), "Logging Errors Page is not loaded in limited time.");

            Assert.That(overview.CheckIfElementExists(overview.GetLoggingErrorsTable(), 3), "Logging Errors table is not loaded.");

            /* Failed Email Tab*/
            overview.ClickFailedEmailsTab();
            Assert.That(overview.IsPageLoaded(waitForSeconds), "Failed Email Page is not loaded in limited time.");

            Assert.That(overview.CheckIfElementExists(overview.GetFailedEmailsTable(), 3), "Failed Emails table is not loaded.");
        }
        ...
        private void VerifyAggregateStatisticsMap(Dictionary<string, string> theMap)
        {
            ...
            Assert.That(foundError, Is.False, "Found error in map, please see console output for the error.");
        }
    }
}
